...
function replaceEnumAndTypedef(asts) {
  asts.forEach((ast) => {
    ...
    (new Map([...ast.structs, ...ast.exceptions])).forEach((struct) => {
...
function getNamespace(headers, filePath, basePath) {
  ...
  if (namespaceHeader) {
    return namespaceHeader.identifier;
  }
  return path.relative(basePath, filePath).replace(/\.thrift$/, '').replace(path.sep, '.');
}
