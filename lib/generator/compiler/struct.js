 ...
 * @param {boolean} scoped is export
 * @param {Object} options
 * @param {string} options.moduleFormat 'cjs/esm'
 * @param {Function} options.fieldFormatter format struct field name
 * @param {Object} options.keywords format struct field name
 */
function structGenerator(name, extend, fieldAST, scoped, options) {
  const typeConvertKey = (options && options.keywords && options.keywords.convertToTypedValueSymbol) || '$convertToTypedValue';
