 ...
 * @param {Map} context
 */
function readMap(keyTypeClass, valueTypeClass, protocol, context) {
  const { ktype, vtype, size } = protocol.readMapBegin();
  for (let i = 0; i < size; ++i) {
    const key = readFieldValue(ktype, keyTypeClass, protocol);
    const value = readFieldValue(vtype, valueTypeClass, protocol);
 ...
 * @param {Set} context
 */
function readSet(TypeClass, protocol, context) {
  const { etype, size } = protocol.readSetBegin();
  for (let i = 0; i < size; ++i) {
    const value = readFieldValue(etype, TypeClass, protocol);
 ...
 * @param {Array} context
 */
function readList(TypeClass, protocol, context) {
 ...
 * @param {Object} FieldTypeClass
 * @param {Object} protocol
 */
function readFieldValue(fieldType, FieldTypeClass, protocol) {
  if (!isSameThriftType(fieldType, FieldTypeClass)) {
  ...
  switch (fieldType) {
    ...
    case Thrift.Type.LIST:
      val = new FieldTypeClass(null, FieldTypeClass.valueType);
      val[read](protocol);
      break;
    ...
    case Thrift.Type.MAP:
      val = new FieldTypeClass(null, FieldTypeClass.keyType, FieldTypeClass.valueType);
      val[read](protocol);
      break;
    case Thrift.Type.STRUCT:
      val = new FieldTypeClass();
      val[read](protocol);
      break;
    default:
      protocol.skip(fieldType);
